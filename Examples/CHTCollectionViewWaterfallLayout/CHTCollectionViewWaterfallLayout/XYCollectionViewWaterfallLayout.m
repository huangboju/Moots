...
NSString *const XYCollectionElementKindSectionHeader = @"XYCollectionElementKindSectionHeader";
NSString *const XYCollectionElementKindSectionFooter = @"XYCollectionElementKindSectionFooter";

@interface XYCollectionViewWaterfallLayout ()
/// The delegate will point to collection view's delegate automatically.
@property (nonatomic, weak) id <XYCollectionViewDelegateWaterfallLayout> delegate;
...
@implementation XYCollectionViewWaterfallLayout
...
- (void)setItemRenderDirection:(XYCollectionViewWaterfallLayoutItemRenderDirection)itemRenderDirection {
...
- (id <XYCollectionViewDelegateWaterfallLayout> )delegate {
    return (id <XYCollectionViewDelegateWaterfallLayout> )self.collectionView.delegate;
}
...
- (void)commonInit {
    ...
    _itemRenderDirection = XYCollectionViewWaterfallLayoutItemRenderDirectionShortestFirst;
}
...
- (void)prepareLayout {
    ...
    NSAssert([self.delegate conformsToProtocol:@protocol(XYCollectionViewDelegateWaterfallLayout)], @"UICollectionView's delegate should conform to XYCollectionViewDelegateWaterfallLayout protocol");
    ...
    for (NSInteger section = 0; section < numberOfSections; ++section) {
        ...
        if (headerHeight > 0) {
            attributes = [UICollectionViewLayoutAttributes layoutAttributesForSupplementaryViewOfKind:XYCollectionElementKindSectionHeader withIndexPath:[NSIndexPath indexPathForItem:0 inSection:section]];
        ...
        if (footerHeight > 0) {
            attributes = [UICollectionViewLayoutAttributes layoutAttributesForSupplementaryViewOfKind:XYCollectionElementKindSectionFooter withIndexPath:[NSIndexPath indexPathForItem:0 inSection:section]];
...
- (NSArray *)layoutAttributesForElementsInRect:(CGRect)rect {
        ...
        if (CGRectIntersectsRect(rect, attr.frame)) {
            switch (attr.representedElementCategory) {
                case UICollectionElementCategorySupplementaryView:
                    if ([attr.representedElementKind isEqualToString:XYCollectionElementKindSectionHeader]) {
                        supplHeaderAttrDict[attr.indexPath] = attr;
                    } else if ([attr.representedElementKind isEqualToString:XYCollectionElementKindSectionFooter]) {
    ...
    if (self.sectionHeadersPinToVisibleBounds) {
        for (UICollectionViewLayoutAttributes *attr in self.allItemAttributes) {
            if ([attr.representedElementKind isEqualToString:XYCollectionElementKindSectionHeader]) {
                [self adjustHeaderAttributesIfNeeded:attr];
                supplHeaderAttrDict[attr.indexPath] = attr;
            }
        }
    }
    if (self.sectionFootersPinToVisibleBounds) {
        for (UICollectionViewLayoutAttributes *attr in self.allItemAttributes) {
            if ([attr.representedElementKind isEqualToString:XYCollectionElementKindSectionFooter]) {
                [self adjustFooterAttributesIfNeeded:attr];
                supplFooterAttrDict[attr.indexPath] = attr;
            }
        }
    }
...
- (NSUInteger)nextColumnIndexForItem:(NSInteger)item inSection:(NSInteger)section {
    ...
    switch (self.itemRenderDirection) {
        case XYCollectionViewWaterfallLayoutItemRenderDirectionShortestFirst:
            index = [self shortestColumnIndexInSection:section];
            break;
            
        case XYCollectionViewWaterfallLayoutItemRenderDirectionLeftToRight:
            index = (item % columnCount);
            break;
            
        case XYCollectionViewWaterfallLayoutItemRenderDirectionRightToLeft:
